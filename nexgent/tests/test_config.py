...
class TestConfigEnvVars:
    def test_config_env_vars(self, monkeypatch):
        ...
        importlib.reload(nexgent.config)
        assert nexgent.config.MIMO_BASE_URL == "http://custom.api.com/v1"
        assert nexgent.config.MIMO_API_KEY == "my-secret-key"
        assert nexgent.config.MIMO_MODEL == "my-custom-model"

    def test_config_defaults(self):
        assert nexgent.config.MIMO_MODEL  # not empty
        assert nexgent.config.MIMO_BASE_URL  # not empty
...
class TestRequireApiKey:
    ...
    def test_require_api_key_missing(self, monkeypatch):
        monkeypatch.setattr(nexgent.config, "MIMO_API_KEY", "")
...
class TestConfigDefaults:
    ...
    def test_default_base_url(self, monkeypatch):
        ...
        importlib.reload(nexgent.config)
        assert "mimo" in nexgent.config.MIMO_BASE_URL.lower() or "api" in nexgent.config.MIMO_BASE_URL.lower()

    def test_default_model(self, monkeypatch):
        monkeypatch.delenv("MIMO_MODEL", raising=False)
        importlib.reload(nexgent.config)
        assert "mimo" in nexgent.config.MIMO_MODEL.lower()

    def test_env_override(self, monkeypatch):
        monkeypatch.setenv("MIMO_MODEL", "custom-model-v1")
        importlib.reload(nexgent.config)
        assert nexgent.config.MIMO_MODEL == "custom-model-v1"
