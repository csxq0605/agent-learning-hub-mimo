        ...
        with self._lock:
            for job in list(self._jobs.values()):
                ...
                if now - job.last_fired < 30:
                    continue
                try:
                    matches = _match_cron(job.cron_expr, now_struct)
                except ValueError:
...
def cron_create(params: dict) -> str:
    ...
    # Validate field values at creation time to prevent background checker crash
    try:
        _parse_cron_field(fields[0], 0, 59)  # minute
        _parse_cron_field(fields[1], 0, 23)  # hour
        _parse_cron_field(fields[2], 1, 31)  # day-of-month
        _parse_cron_field(fields[3], 1, 12)  # month
        _parse_cron_field(fields[4], 0, 6)   # day-of-week (0=Sun)
